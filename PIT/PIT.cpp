...
int *key, noOfBits, noOfBitsEmbedded = 0, noOfPixels=0;
bool *secretBits;
...
{
	int j,k;
	string path = homedir + "/IS/PITTool";
...
IplImage* CarrierEngine ( IplImage* img )
{
	...
	int i, j, k, l, m, n, col, colBin[8], p, mc[2], colBin1[8];
	...
	//add noOfBits to 0th key
	l = noOfBits / 255;
	noOfBitsEmbedded = noOfBits;
	
	for (m=0; m<=l; m++) {
		if (m == l) {
			data[i++] = static_cast <unsigned char>(noOfBitsEmbedded);
			noOfBitsEmbedded = 0;
		}
		else {
	...
	while (noOfBitsEmbedded < noOfBits) {
		...
		if (colBin1[1] == 1) {
			...
			data[j+k+mc[0]] = static_cast <unsigned char>(bin2dec(colBin));
		}
		if (colBin1[0] == 1) {
			col = data[j+k+mc[1]];
			for (n=0; n<8; n++) {
			...
			data[j+k+mc[1]] = static_cast <unsigned char>(bin2dec(colBin));
		}
		i++;
	}
...
void CarrierEngineExtract ( IplImage* img )
{
	int width = img->width, height = img->height, step = img->widthStep;
	nChannels = img->nChannels;
	int i, j, k, l, m, n, col, colBin[8], colBin1[8], mc[2], p;
